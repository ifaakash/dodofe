...
const StatsCard = ({ userInvoicesData, setTimePeriod, timePeriod }: StatsCardProps) => {  
  ...
  return (
    <div className="px-5 flex flex-col gap-2">
      <div className="bg-white rounded-xl p-4 flex flex-col gap-4">
        <TabSwitch timeRange={timePeriod} setTimeRange={setTimePeriod} timeRangeOptions={TIME_PERIODS.map((time) => ({ label: time.label, value: time.value }))} handleTimeRangeChange={handleTimePeriodChange} />


        <div className="flex flex-col gap-3">
          <div className="flex justify-center relative">
            {
              userInvoicesData ? (<HalfDonutChart
                paidAmount={userInvoicesData?.paidAmount ?? 0}
                pendingAmount={userInvoicesData?.unpaidAmount ?? 0}
              />) : (
                <div className="flex justify-center items-center h-[200px]">
                  <div className="text-sm font-medium">Not Enough Data Available</div>
                </div>
              )
            }
            <div className="flex flex-col gap-0.5 absolute bottom-0 items-center">
              ...
              <div className="font-semibold text-xl">
                ₹{((userInvoicesData?.outStandingAmount ?? 0) + (userInvoicesData?.unpaidAmount ?? 0)).toLocaleString('en-IN')}
              </div>
            </div>
          </div>
          <div className="flex justify-between">
            <div className="flex flex-col gap-0.5">
              ...
              <div className="text-[#3D4966] font-semibold">
                ₹{userInvoicesData?.paidAmount?.toLocaleString('en-IN') ?? '--'}
              </div>
            </div>
            <div className="flex flex-col gap-0.5">
              ...
              <div className="text-[#3D4966] font-semibold text-end">
                ₹{userInvoicesData?.unpaidAmount?.toLocaleString('en-IN') ?? '--'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-2">
        <Link href={"/invoice/history?mode=all"} className="bg-white w-full rounded-xl flex flex-col justify-between p-3 text-center">
          <div className="text-xl font-semibold text-[#414D55]">{userInvoicesData?.invoices.created ? userInvoicesData?.invoices.created : '--'}</div>
        ...
        <Link href={"/invoice/history?mode=paid"} className="bg-white w-full rounded-xl flex flex-col justify-between p-3 text-center">
          <div className="text-xl font-semibold text-[#414D55]">{userInvoicesData?.invoices.paid ? userInvoicesData?.invoices.paid : '--'}</div>
          <div className="flex gap-2 items-center justify-center">
            <div>Paid</div>
        ...
        <Link href={"/invoice/history?mode=due"} className="bg-white w-full rounded-xl flex flex-col justify-between p-3 text-center">
          <div className="text-xl font-semibold text-[#414D55]">{userInvoicesData?.invoices.due ? userInvoicesData?.invoices.due : '--'}</div>
          <div className="flex gap-2 items-center justify-center">
            <div>Due</div>
