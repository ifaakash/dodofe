...
const HeroSection = ({
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (
                ...
                const audioBioButton =
                    document.getElementById("audio-bio-button");
    ...
    const handleImageUpload = async () => {
        ...
        if (result.success && result.file && result.previewUrl) {
            setImagePreview(result.previewUrl);
            dispatch(updateDodoPageProfilePicture(result.file));
    ...
    useEffect(() => {
        const messageHandler = async (event) => {
            if (isWebview()) {
                try {
                    const { action, status, base64, mimeType } = JSON.parse(
                        event.data
                    );
                    ...
                    if (action === "audioPermissionResponse") {
                        if (status === "granted") {
                            window.ReactNativeWebView?.postMessage(
                                JSON.stringify({ action: "startRecording" })
                            );
    ...
    const playAudio = () => {
        ...
        if (!audioRef.current) {
            ...
            // iOS requires these to be set before play() is called
            audio.preload = "auto";
            audio.load();
        ...
        if (isPlaying) {
            ...
            // Use a promise chain with explicit user interaction handling
    ...
    const playProfileAudio = () => {
        ...
        try {
            ...
            if (!profileAudioRef.current) {
                ...
                // iOS requires these settings before play() is called
                profileAudioRef.current.preload = "auto";

                // Set the source
                profileAudioRef.current.src = dodoPageDetails.audioBio;

                // Load the audio (important for iOS)
                ...
                // Add error handling
                profileAudioRef.current.onerror = (e) => {
            ...
            if (isProfileAudioPlaying) {
                ...
                const playPromise = profileAudioRef.current.play();
                ...
                if (playPromise !== undefined) {
                    playPromise
                        ...
                        .catch((err) => {
                            console.error("Profile audio playback error:", err);
                            setIsProfileAudioPlaying(false);

                            // Try to recreate the audio element on error
                            if (profileAudioRef.current) {
                                profileAudioRef.current = null;
                                // Retry playback after a short delay
                                setTimeout(() => playProfileAudio(), 100);
                            }
                        });
                }
            }
        } catch (err) {
            console.error("Error in profile audio playback:", err);
            setIsProfileAudioPlaying(false);
        }
    };
                        ...
                        {mode === "edit" && (
                            ...
                            >
                                <div className="text-xs font-medium text-[#414D55]">
                                    {dodoPageDetails.audioBio ? "Edit" : "Add"}{" "}
                                    Audio Bio
                                </div>
                                <Image
                                    src={PlusCircle}
                    ...
                    <div className="fixed inset-x-0 bottom-0 z-50 p-2">
                        <div className="flex justify-center">
                            ...
                            >
                                <X
                                    size={26}
                                    className="cursor-pointer text-brandPrimary"
                                />
                            </div>
                        </div>
                        <div
                  ...
                  <span className="text-brandPrimary text-[8px]">upload</span>
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    ...
                                    <Image
                                        src={
                                            audioBlob ? SpeakerActive : Speaker
                                        }
                                        alt="Speaker"
                                    />
                                </button>
