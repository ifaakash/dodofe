...
const PollResponses = ({ pollData }: { pollData: any }) => {
  ...
  const totalVotes = Object.values(pollData?.optionCounts || {}).reduce<number>(
  ...
  return (
    <div className="bg-white px-4 py-3 rounded-xl flex flex-col gap-4 w-full">
      ...
      <div className="flex gap-6">
        <div className="w-32">
          <FullDonutChart 
            OptionA={getOptionPercentage(pollData?.options[0])}
            OptionB={getOptionPercentage(pollData?.options[1])}
            OptionC={getOptionPercentage(pollData?.options[2])}
            OptionD={getOptionPercentage(pollData?.options[3])}
          />
        </div>
