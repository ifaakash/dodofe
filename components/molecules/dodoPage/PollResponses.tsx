...
const PollResponses = ({ pollData }: { pollData: any }) => {
  ...
  const getOptionPercentage = (option: string): number => {
    const optionVotes = Number(getOptionVotes(option)) || 0;
    const votes = Number(totalVotes) || 0;

    if (votes === 0) return 0;

    return Math.round((optionVotes / votes) * 100);
  };
  ...
  return (
    <div className="bg-white px-4 py-3 rounded-xl flex flex-col gap-4 w-full">
      <div className="flex justify-between">
        ...
        <div className="font-semibold">{totalVotes as number}</div>
      </div>
          ...
          {pollData?.options.map((option: string, index: number) => (
            <div key={index} className="flex justify-between items-center w-full">
              <div className="flex gap-1 items-center">
                <div className={`w-[10px] h-[10px] rounded-full ${index === 0 ? 'bg-[#FB7053]' :
                  index === 1 ? 'bg-[#F1C400]' :
                    index === 2 ? 'bg-[#42ADD9]' :
                      'bg-[#967BDD]'
                  }`}></div>
      ...
      <div className="flex justify-between gap-1">
        {pollData?.options.map((option: string, index: number) => (
          <div
            key={index}
            className={`w-full py-1 px-2 rounded-[4px] text-white text-center font-semibold ${index === 0 ? 'bg-[#FB7053]' :
              index === 1 ? 'bg-[#F1C400]' :
                index === 2 ? 'bg-[#42ADD9]' :
                  'bg-[#967BDD]'
              }`}
