    ...
    const handlePublish = async () => {
        try {
            if (blockState.newBlocksAdded) {
                    ...
                    try {
                        switch (block.blockType) {
                            case "LINK":
                                ...
                                const createLink = await createBlockWithMedia(
                                    formData
                                );
                            ...
                            case "PRODUCT":
                                ...
                                const createProduct =
                                    await createBlockWithMedia(formDataProduct);
                ...
                // Update all block IDs at once
                for (const update of blockIdUpdates) {
                    dispatch(updateBlockId(update));
                }
            ...
            if (dodoPageState.unsavedChanges) {
                ...
                if (dodoPageState.isSocialLinksChanged) {
                    ...
                    Object.entries(dodoPageState.socialLinks).forEach(
                        ([platform, url]) => {
                            formData.append(
                    ...
                    try {
                        switch (block.blockType) {
                            ...
                            case "PRODUCT":
                                ...
                                const productRes = await updateBlockWithMedia(
                                    formDataProduct
                                );
                            ...
                            case "LINK":
                                ...
                                const linkRes = await updateBlockWithMedia(
                                    formDataLink
                                );
        ...
        <div>
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className="bg-white shadow-md border-[1px] py-[14px] px-[10px] rounded-full w-full flex text-sm font-semibold items-center justify-center text-brandPrimary backdrop-filter backdrop-blur-sm bg-white/70"
                >
                    Analytics
                </button>
