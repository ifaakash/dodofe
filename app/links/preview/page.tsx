...
function Preview({
    ...
    const getHeader = () => {
        return (
            <div className="flex flex-row justify-between items-center w-full">
                <div className="flex items-center" onClick={() => router.push(ROUTE_CONSTANTS.LINKS + `?userId=${userId}`)}>
                    <Image
                        height={16}
                        width={16}
                        src={leftArrow}
                        ...
                        onClick={() => router.back()}
                    />
                    <span className="ml-2 clr-text text-sm">Go to edit mode</span>
                </div>
            </div >
        );
    };
    ...
    return (
        <div className={cx("mx-4 mt-4 flex flex-col items-center bioBg", className)}>
            {getHeader()}
                ...
                return (
                    <>
                        {data?.type === BLOCKS.LINK && (
                            <div key={data?.description} onClick={() => { gotoLink(data?.url) }} className={cx("w-full h-14 bg-white rounded-xl mb-4 flex items-center justify-between shadow-md linkCard", styles.lightBorder)}>
                        ...
                        {data?.type === BLOCKS.HEADING && <h2 className="absolute-center clr-heading-text text-lg font-regular">{data?.description}</h2>}
                    </>
                );
            })}
            ...
            <Button
                ...
                endIcon={rightArrow}
            />
            {/* <div className="absolute w-full bottom-2 font-light flex justify-center">
        Crafted with ❤️ in <span className="font-semibold ml-1">BHARAT</span>
      </div> */}
        </div >
    );
}
